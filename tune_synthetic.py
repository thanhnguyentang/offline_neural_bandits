    ...
    if algo_group == 'approx-neural':
        for lr in [1e-4,1e-3]:
            for batch_size,num_steps,buffer_s in [(1,1,1),(50,100,-1)]:
                for beta in [0.01, 0.05, 1,5,10]: #[0.01, 0.05, 0.1, 0.5, 1, 5]:
                    commands.append('python synthetic_main.py --data_type {} --algo_group {} --num_sim {} --batch_size {} --num_steps {} --buffer_s {} --beta {} --lr {}'.format(data_type,algo_group,num_sim,batch_size,num_steps,buffer_s,beta,lr))

    elif algo_group == 'neural-greedy':
        for lr in [1e-4,1e-3]:
            for batch_size,num_steps,buffer_s in [(1,1,1),(50,100,-1)]:
                commands.append('python synthetic_main.py --data_type {} --algo_group {} --num_sim {} --batch_size {} --num_steps {} --buffer_s {} --lr {}'.format(data_type,algo_group,num_sim,batch_size,num_steps,buffer_s,lr))
